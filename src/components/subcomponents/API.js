import React, { useState } from "react";
import axios from "axios";
import { Loader } from "../functional/Loader";

class Compost extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: true,
      data: [],
      index: 0,
    };
  }

  componentDidMount() {
    let config = {
      headers: {
        Authorization:
          "Bearer ",
      },
    };

    axios.get("http://localhost:1337/api/posts", config).then((res) => {
      let data = res.data;
      this.setState({ data, loading: false });
      console.log(data);
    });
  }

  increment = () => {
    if (this.state.index >= 4) {
      this.setState((state) => {
        return { index: (state.index - 3) };
      });
    } else {
      this.setState((state) => {
        return { index: state.index + 1 };
      });
    }
  };

  render() {
    if (this.state.loading) {
      return <Loader />;
    } else {
      console.log(this.state.index);
      setTimeout(this.increment, 5000);

      return (
        <div className="text-center text-white">
          <div className="font-semibold text-4xl animate-fade-in-up">
            {this.state.data.data[this.state.index].attributes.Title}
          </div>
          <div className="text-3xl animate-fade-in-down"></div>
          {this.state.data.data[this.state.index].attributes.Body}
          <div className="text-3xl font-semibold"></div>
        </div>
      );
    }
  }
}

export { Compost };
